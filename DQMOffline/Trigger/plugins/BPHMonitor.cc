...
//
  double PrescaleWeight =1, PrescaleWeight_den =1;
  ...
  int  PrescaleHLT=1, PrescaleHLT1=1;
  int PrescaleL1_1 = 1, PrescaleL1_2=1,PrescaleL1=1;
  ...
  if (num_genTriggerEventFlag_->on() &&  num_genTriggerEventFlag_->accept( iEvent, iSetup) )
  {
    ...
    PrescaleL1_1 = (hltPrescale_->prescaleValuesInDetail(iEvent, iSetup, hltpath1)).first.at(0).second;//
    if ((hltPrescale_->prescaleValuesInDetail(iEvent, iSetup, hltpath1)).first.size()>1) PrescaleL1_2 = (hltPrescale_->prescaleValuesInDetail(iEvent, iSetup, hltpath1)).first.at(1).second;//
    else PrescaleL1_2=0;//only 1 L1 seed 
    std::cout<<"L1_1 prescale = "<<PrescaleL1_1<<std::endl;
    std::cout<<"L1_2 prescale = "<<PrescaleL1_2<<std::endl;
    if (PrescaleHLT<0)
    { 
      PrescaleHLT=1;
      std::cout<<"negative HLT Prescale"<<std::endl;
    }
    if (PrescaleL1_1<0)
    {
      PrescaleL1_1=1;
      std::cout<<"negative L1_1 Prescale"<<std::endl;
    }
  
    if (PrescaleL1_2<0)
    {
      PrescaleL1_2=1;
      std::cout<<"negative L1_2 Prescale"<<std::endl;
    }
    if(PrescaleL1_1>1 && PrescaleL1_2>1) PrescaleL1 = (int)((PrescaleL1_1*PrescaleL1_2)/((PrescaleL1_1*PrescaleL1_2) - ((PrescaleL1_1-1)*(PrescaleL1_2-1))));
    else
    {
      if(PrescaleL1_2>0)PrescaleL1=1;
      else PrescaleL1=PrescaleL1_1;//only one L1
    }
  ...
  {
    PrescaleHLT = (hltPrescale_->prescaleValuesInDetail(iEvent, iSetup, hltpath)).second;//
    PrescaleL1_1 = (hltPrescale_->prescaleValuesInDetail(iEvent, iSetup, hltpath)).first.at(0).second;//
    if ((hltPrescale_->prescaleValuesInDetail(iEvent, iSetup, hltpath)).first.size()>1) PrescaleL1_2 = (hltPrescale_->prescaleValuesInDetail(iEvent, iSetup, hltpath)).first.at(1).second;//
    else PrescaleL1_2=0;//only 1 L1 seed 
    std::cout<<"L1_1 prescale = "<<PrescaleL1_1<<std::endl;
    std::cout<<"L1_2 prescale = "<<PrescaleL1_2<<std::endl;
    if (PrescaleHLT<0)
    ...
    PrescaleWeight_den = PrescaleL1*PrescaleHLT;
    std::cout<<"total L1 prescale den= "<<PrescaleL1<<std::endl;
    std::cout<<"total HLT prescale den= "<<PrescaleHLT<<std::endl;
    std::cout<<"total prescale den= "<<PrescaleWeight_den<<std::endl;
    ...
    for (int i = 0; i<int(tagMuons.size());i++){
      for ( auto const & m : *muoHandle ) { 
        ...
        if ((tagMuons[i].p4()+m.p4()).M() >minmass_&& (tagMuons[i].p4()+m.p4()).M() <maxmass_){//near to J/psi mass
          muPhi_.denominator->Fill(m.phi(),PrescaleWeight_den);
          muEta_.denominator->Fill(m.eta(),PrescaleWeight_den);
          muPt_.denominator ->Fill(m.pt(),PrescaleWeight_den);
    ...
    for (auto const & m : *muoHandle ) {
      ...
      for (auto const & m1 : *muoHandle ) {
        ...
        case 2:
          ...
          mu1Phi_.denominator->Fill(m.phi(),PrescaleWeight_den);
          mu1Eta_.denominator->Fill(m.eta(),PrescaleWeight_den);
          mu1Pt_.denominator ->Fill(m.pt(),PrescaleWeight_den);
          mu2Phi_.denominator->Fill(m1.phi(),PrescaleWeight_den);
          mu2Eta_.denominator->Fill(m1.eta(),PrescaleWeight_den);
          mu2Pt_.denominator ->Fill(m1.pt(),PrescaleWeight_den);
          DiMuPt_.denominator ->Fill((m1.p4()+m.p4()).Pt() ,PrescaleWeight_den);
          DiMuEta_.denominator ->Fill((m1.p4()+m.p4()).Eta() ,PrescaleWeight_den);
          DiMuPhi_.denominator ->Fill((m1.p4()+m.p4()).Phi(),PrescaleWeight_den);
          if (num_genTriggerEventFlag_->on() &&  num_genTriggerEventFlag_->accept( iEvent, iSetup) )
          {
...
//            
        ...
        case 4:
         ...
         //std::cout<<"inside num"<<enum_<<std::endl;
          ...
          DiMuEta_.denominator ->Fill((m1.p4()+m.p4()).Eta() ,PrescaleWeight_den);
          DiMuPhi_.denominator ->Fill((m1.p4()+m.p4()).Phi(),PrescaleWeight_den);
          DiMudR_.denominator ->Fill(reco::deltaR(m,m1),PrescaleWeight_den);
          if (num_genTriggerEventFlag_->on() &&  num_genTriggerEventFlag_->accept( iEvent, iSetup) )
          {
         //std::cout<<"inside num"<<enum_<<std::endl;
            //const std::string const std::string & hltpath1 hltpath1 = hltpaths_num[0];
        ...
        case 6: 
          ...
          for (auto const & m2 : *muoHandle) {//triple muon paths
            ...
            mu2Eta_.denominator->Fill(m1.eta(),PrescaleWeight_den);
            mu2Pt_.denominator ->Fill(m1.pt(),PrescaleWeight_den);
            mu3Phi_.denominator->Fill(m2.phi(),PrescaleWeight_den);
            mu3Eta_.denominator->Fill(m2.eta(),PrescaleWeight_den);
            mu3Pt_.denominator ->Fill(m2.pt(),PrescaleWeight_den);
            if (num_genTriggerEventFlag_->on() &&  num_genTriggerEventFlag_->accept( iEvent, iSetup) )
            {
           //std::cout<<"inside num"<<enum_<<std::endl;
              //const std::string const std::string & hltpath1 hltpath1 = hltpaths_num[0];
        ...
        case 7:// the hists for photon monitoring will be filled on 515 line
          ...
          { 
            ...
            if(false && !matchToTrigger(hltpath,p, handleTriggerEvent)) continue;
              phPhi_.denominator->Fill(p.phi(),PrescaleWeight_den);
              phEta_.denominator->Fill(p.eta(),PrescaleWeight_den);
              phPt_.denominator ->Fill(p.pt(),PrescaleWeight_den);
              if (num_genTriggerEventFlag_->on() &&  num_genTriggerEventFlag_->accept( iEvent, iSetup) )
              {
             //std::cout<<"inside num"<<enum_<<std::endl;
                //const std::string const std::string & hltpath1 hltpath1 = hltpaths_num[0];
        ...
        case 8://vtx monitoring, filling probability, DS, DCA, cos of pointing angle to the PV, eta, pT of dimuon
          ...
          DiMuProb_.denominator ->Fill( dimuonCL,PrescaleWeight_den);
         ...
         //std::cout<<"inside num"<<enum_<<std::endl;
          ...
          if (dimuonCL<minprob)continue;
          DiMuDS_.denominator ->Fill( displacementFromBeamspotJpsi.perp()/sqrt(jerr.rerr(displacementFromBeamspotJpsi)),PrescaleWeight_den);
          DiMuPVcos_.denominator ->Fill(jpsi_cos ,PrescaleWeight_den);
          DiMuPt_.denominator ->Fill((m1.p4()+m.p4()).Pt() ,PrescaleWeight_den);
          DiMuEta_.denominator ->Fill((m1.p4()+m.p4()).Eta() ,PrescaleWeight_den);
          DiMuDCA_.denominator ->Fill( cApp.distance(),PrescaleWeight_den);
          if (num_genTriggerEventFlag_->on() &&  num_genTriggerEventFlag_->accept( iEvent, iSetup) )
          {
         //std::cout<<"inside num"<<enum_<<std::endl;
            //const std::string const std::string & hltpath1 hltpath1 = hltpaths_num[0];
          ...
          { 
            ...
            float jpsiKcos = vperp.Dot(pperp)/(vperp.R()*pperp.R());
	          ...
	          muPhi_.denominator->Fill(t.phi(),PrescaleWeight_den);
    	      muEta_.denominator->Fill(t.eta(),PrescaleWeight_den);
	          muPt_.denominator ->Fill(t.pt(),PrescaleWeight_den);
            if (num_genTriggerEventFlag_->on() &&  num_genTriggerEventFlag_->accept( iEvent, iSetup) )
            {
         //std::cout<<"inside num"<<enum_<<std::endl;
              //const std::string const std::string & hltpath1 hltpath1 = hltpaths_num[0];
    	...
    	case 10:
          ...
          double MuMass2 = mu_mass * mu_mass;//0.1056583745 *0.1056583745;
  	      ...
  	      reco::Vertex::Point vperp(displacementFromBeamspot.x(),displacementFromBeamspot.y(),0.);
  	      if (JpsiTkCL<minprob)continue;
  	      muPhi_.denominator->Fill(m1.phi(),PrescaleWeight_den);
  	      muEta_.denominator->Fill(m1.eta(),PrescaleWeight_den);
  	      muPt_.denominator ->Fill(m1.pt(),PrescaleWeight_den);
          if (num_genTriggerEventFlag_->on() &&  num_genTriggerEventFlag_->accept( iEvent, iSetup) )
          {
         //std::cout<<"inside num"<<enum_<<std::endl;
            //const std::string const std::string & hltpath1 hltpath1 = hltpaths_num[0];
  	    ...
  	    for (auto const & t : *trHandle) {
  	      ...
  	      if(false && !matchToTrigger(hltpath,t, handleTriggerEvent)) continue;
              ...
              double MuMass2 = mu_mass * mu_mass;//0.1056583745 *0.1056583745;
          		...
          		if (fabs(jpsiKcos)<mincos)continue;
          		if ((displacementFromBeamspot.perp()/sqrt(err.rerr(displacementFromBeamspot)))<minDS)continue;
          		mu1Phi_.denominator->Fill(t.phi(),PrescaleWeight_den);
          		mu1Eta_.denominator->Fill(t.eta(),PrescaleWeight_den);
          		mu1Pt_.denominator ->Fill(t.pt(),PrescaleWeight_den);
          		mu2Phi_.denominator->Fill(t1.phi(),PrescaleWeight_den);
          		mu2Eta_.denominator->Fill(t1.eta(),PrescaleWeight_den);
          		mu2Pt_.denominator ->Fill(t1.pt(),PrescaleWeight_den);
              if (num_genTriggerEventFlag_->on() &&  num_genTriggerEventFlag_->accept( iEvent, iSetup) )
              {
                //std::cout<<"inside num"<<enum_<<std::endl;
                //const std::string const std::string & hltpath1 hltpath1 = hltpaths_num[0];
