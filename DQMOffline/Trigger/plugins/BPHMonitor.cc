...
{  
  ...
  if (tnp_) istnp = "Tag_and_Probe/";
  else istnp = "";
  std::string currentFolder = folderName_ + istnp;
  ibooker.setCurrentFolder(currentFolder.c_str());

  if (trOrMu_) trMuPh = "tr";
  else if (Ph_) trMuPh = "ph";
  else trMuPh = "mu";
...
void BPHMonitor::analyze(edm::Event const& iEvent, edm::EventSetup const& iSetup)  {
  ...
  if (tnp_>0) {//TnP method 
    ...
    iEvent.getByToken( hltInputTag_, handleTriggerEvent);
    iEvent.getByToken( hltTrigResTag_, handleTriggerTrigRes);
    if (enum_==9 && handleTriggerTrigRes->accept(285)) cout<<hltpaths_den[0]<<" TRIGGERED!!!"<<endl;
    // if (enum_==9) for (int i=0; i<(int)handleTriggerTrigRes->size(); ++i) if (handleTriggerTrigRes->accept(i)) cout<<i<<"\t"<<handleTriggerTrigRes->accept(i)<<endl;
    // cout<<hltpaths_den[0]<<"\t"<<handleTriggerEvent->sizeCollections()<<"\t"<<handleTriggerEvent->sizeFilters()<<"\t"<<handleTriggerEvent->sizeObjects()<<endl;
    if (den_genTriggerEventFlag_->on() && ! den_genTriggerEventFlag_->accept( iEvent, iSetup) ) return;
    cout<<hltpaths_den[0]<<"\t\taccepted"<<endl;
    if (handleTriggerEvent->sizeFilters()== 0) return;
    const std::string & hltpath = hltpaths_den[0];
    bool denFilled = false;
    for (auto const & m : *muoHandle ) {
      if (!muoSelection_ref(m)) {cout<<hltpaths_den[0]<<"\t\t!muoSelection_ref (m)"<<endl; continue;}
      if (false && !matchToTrigger(hltpath,m, handleTriggerEvent)) {cout<<hltpaths_den[0]<<"\t\tfalse && !matchToTrigger (m)"<<endl; continue;}
      for (auto const & m1 : *muoHandle ) {
        if (m1.pt() == m.pt()) continue;
        if (!muoSelection_ref(m1)) {cout<<hltpaths_den[0]<<"\t\t!muoSelection_ref (m1)"<<endl; continue;}   
        if (false && !matchToTrigger(hltpath,m1, handleTriggerEvent)) {cout<<hltpaths_den[0]<<"\t\tfalse && !matchToTrigger (m1)"<<endl; continue;}
	cout<<hltpaths_den[0]<<"\t\tDOUBLE MATCH!"<<endl;
        if (enum_<10){
	...
	cout<<hltpaths_den[0]<<"\t\tto selection "<<enum_<<endl;
        ...
        case 2:
	  cout<<hltpaths_den[0]<<"\t\t case2"<<endl;
          if ((Jpsi_) && (!Upsilon_)){
          ...
          break;
        case 3:
	  cout<<hltpaths_den[0]<<"\t\t case3"<<endl;
          if ((Jpsi_) && (!Upsilon_)){
            if (DiMuMass> maxmassJpsi || DiMuMass< minmassJpsi)continue;
          }
          if ((!Jpsi_) && (Upsilon_)){
          ...
          break; 
        case 4:
	  cout<<hltpaths_den[0]<<"\t\t case4"<<endl;
          if (dimuonCL<minprob)continue;
          ...
          break;
        case 5:
	  cout<<hltpaths_den[0]<<"\t\t case5"<<endl;
          if (dimuonCL<minprob)continue;
          if ((Jpsi_) && (!Upsilon_)){
            if (DiMuMass> maxmassJpsi || DiMuMass< minmassJpsi)continue;
          }
          ...
          break;
        case 6: 
	  cout<<hltpaths_den[0]<<"\t\t case6"<<endl;
          if (dimuonCL<minprob)continue;
          if ((Jpsi_) && (!Upsilon_)){
            if (DiMuMass> maxmassJpsi || DiMuMass< minmassJpsi)continue;
          }
	  ...
	  cout<<hltpaths_den[0]<<"\t\t case8"<<endl;
          if ((Jpsi_) && (!Upsilon_)){
            if (DiMuMass> maxmassJpsi || DiMuMass< minmassJpsi)continue;
          }
          ...
          if (dimuonCL<minprob) {cout<<hltpaths_den[0]<<"\t\t case9 dimuonCL: "<<dimuonCL<<" less than "<<minprob<<endl; continue;}
          ...
          break;
	case 9:
	  cout<<hltpaths_den[0]<<"\t\t case9"<<endl;
          if (dimuonCL<minprob) {cout<<hltpaths_den[0]<<"\t\t case9 dimuonCL: "<<dimuonCL<<" less than "<<minprob<<endl; continue;}
          if (fabs(jpsi_cos)<mincos) {cout<<hltpaths_den[0]<<"\t\t case9 jpsi_cos: "<<jpsi_cos<<" less than "<<mincos<<endl; continue;}
          if ((displacementFromBeamspotJpsi.perp()/sqrt(jerr.rerr(displacementFromBeamspotJpsi)))<minDS) {cout<<hltpaths_den[0]<<"\t\t case9 displacement: "<<displacementFromBeamspotJpsi.perp()<<"/"<<sqrt(jerr.rerr(displacementFromBeamspotJpsi))<<" less than "<<minDS<<endl; continue;}
	  ...
	  if (trHandle.isValid()){
	    ...
	    for (auto const & t : *trHandle) {
	      ...
	      if(!trSelection_ref(t)) {cout<<hltpaths_den[0]<<"\t\t case9 track discarded"<<endl; continue;}
              reco::Track itrk1       = t ;                                                
              
              if((reco::deltaR(t,m1) <= 0.001))continue;//checking overlaping
              if((reco::deltaR(t,m) <= 0.001)) continue;
   
              if (! itrk1.quality(reco::TrackBase::highPurity))      {cout<<hltpaths_den[0]<<"\t\t case9 low purity track"<<endl; continue;}
              ...
              pB   = p1 + p2 + p3;
              if( pB.mass()> maxmassJpsiTk || pB.mass()< minmassJpsiTk) {cout<<hltpaths_den[0]<<"\t\t case9 B.mass: "<<pB.mass()<<" not in range "<<minmassJpsiTk<<" "<<maxmassJpsiTk<<endl; continue;}
              ...
              if (!tv.isValid())  {cout<<hltpaths_den[0]<<"\t\t case9 vertex not valid"<<endl; continue;}
              float JpsiTkCL = 0;
		...
		JpsiTkCL = TMath::Prob(vertex.chi2(), vertex.ndof() );
	      ...
	      if (JpsiTkCL<minprob) {cout<<hltpaths_den[0]<<"\t\t case9 JpsiTkCL: "<<JpsiTkCL<<" less than "<<minprob<<endl; continue;}
	      if (fabs(jpsiKcos)<mincos) {cout<<hltpaths_den[0]<<"\t\t case9 jpsiKcos: "<<jpsiKcos<<" less than "<<mincos<<endl; continue;}
	      if ((displacementFromBeamspot.perp()/sqrt(err.rerr(displacementFromBeamspot)))<minDS) {cout<<hltpaths_den[0]<<"\t\t case9 displacement: "<<(displacementFromBeamspot.perp()/sqrt(err.rerr(displacementFromBeamspot)))<<" less than "<<minDS<<endl; continue;}
	      muPhi_.denominator->Fill(t.phi());
	      muEta_.denominator->Fill(t.eta());
	      muPt_.denominator ->Fill(t.pt());
	      cout<<hltpaths_den[0]<<"\t\t+"<<endl; 
	      /////////////////////////
	    }
	  } else cout<<hltpaths_den[0]<<"\t\t case9 no track handle"<<endl;
	  break;
	case 10:
	  cout<<hltpaths_den[0]<<"\t\t case10"<<endl;
	  if (trHandle.isValid()){
	    for (auto const & t : *trHandle) {
	  ...
	  break;
	case 11:
	  cout<<hltpaths_den[0]<<"\t\t case11"<<endl;
	  if (dimuonCL<minprob)continue;
	...
	if (enum_ == 7){//photons
	  ...
	  for (auto const & p : *phHandle) {
	    ...
	    cout<<hltpaths_den[0]<<"\t\tphoton: "<<p.pt()<<" "<<p.eta()<<" "<<p.phi()<<endl;
	    cout<<phPhi_.denominator->getEntries()<<endl;
	    phPhi_.denominator->Fill(p.phi());
	    cout<<hltpaths_den[0]<<"\t\tphoton: "<<p.pt()<<" "<<p.eta()<<" "<<p.phi()<<endl;
	    cout<<phEta_.denominator->getEntries()<<endl;
	    phEta_.denominator->Fill(p.eta());
	    cout<<hltpaths_den[0]<<"\t\tphoton: "<<p.pt()<<" "<<p.eta()<<" "<<p.phi()<<endl;
	    cout<<phPt_.denominator->getEntries()<<endl;
	...
	denFilled = true;
	cout<<hltpaths_den[0]<<"\t\t FILLED den"<<endl;
      } // mu1 loop
    } // mu loop
    ...
    cout<<hltpaths_num[0]<<" (num)\t\taccepted"<<endl;
    iEvent.getByToken( hltInputTag_, handleTriggerEvent);
    if (handleTriggerEvent->sizeFilters()== 0)return;
...
bool BPHMonitor::matchToTrigger(const std::string  &theTriggerName , T t, edm::Handle<trigger::TriggerEvent> handleTriggerEvent){
  ...
  if (handleTriggerEvent->sizeFilters() >0){
    ...
    cout<<theTriggerName<<"\t\tTrigMatch: "<<handleTriggerEvent->sizeFilters()<<endl;
    for ( size_t ia = 0; ia < handleTriggerEvent->sizeFilters(); ++ ia) {
      ...
      cout<<theTriggerName<<"\t\tTrigMatch "<<ia<<": "<<k.size()<<" "<<fullname<<" => "<<name<<endl<<endl;
      for (trigger::Keys::const_iterator ki = k.begin(); ki !=k.end(); ++ki ) {
	reco::Particle theTriggerParticle = toc[*ki].particle();
	if(name.find(theTriggerName) != string::npos){ cout<<theTriggerName<<"\t\tTrigMatch "<<ia<<": "<<reco::deltaR(t.eta(), t.phi(),theTriggerParticle.eta(),theTriggerParticle.phi())<<endl;
